...
public static class GameEvents
{
    ...
    /// <summary>Fired by SceneInitializer after a scene finishes loading and config is applied.
    /// Other systems (audio, quest, intro triggers) listen to react per-scene.</summary>
    public static event Action<SceneConfig> OnSceneLoaded;

    /// <summary>Fired by any system requesting a scene transition (e.g. quest complete, door trigger).
    /// SceneTransitionManager listens to execute the fade-and-load.</summary>
    ...
    public static void RaiseSceneLoaded(SceneConfig config)              => OnSceneLoaded?.Invoke(config);
    public static void RaiseSceneTransitionRequested(string sceneName)   => OnSceneTransitionRequested?.Invoke(sceneName);
}
